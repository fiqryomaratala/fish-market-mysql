...
import type { FeedingTrendPoint } from '@/types/report'
import { formatDate, formatNumber } from '@/utils/format'

type FeedingChartProps = {
  data: FeedingTrendPoint[]
}
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold text-slate-900">Tren penggunaan pakan</h3>
        </div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600">
          {formatNumber(data.reduce((total, item) => total + item.activities, 0))} aktivitas
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}>
            ...
            <Tooltip
              ...
              formatter={(value) => [`${formatNumber(Number(value))} kg`, 'Pakan dipakai']}
            />
