// Komponen Order Chart untuk Analytics Dashboard
import {
  AreaChart,
  Area,
  XAxis,
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold text-slate-900">Trend pesanan</h3>
        </div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600">
          {formatNumber(data.reduce((total, item) => total + item.orders, 0))} pesanan
        </div>
      </div>
          ...
          <AreaChart data={data}>
            ...
            <Tooltip
              ...
              formatter={(value, name) => {
                if (name === 'orders') return [formatNumber(Number(value)), 'Total Pesanan']
                if (name === 'completed_orders') return [formatNumber(Number(value)), 'Pesanan Selesai']
                return [formatNumber(Number(value)), 'Revenue']
              }}
            />
            <Area
              type="monotone"
